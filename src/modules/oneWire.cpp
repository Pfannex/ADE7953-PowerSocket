...
void OW::scanBus() {
  ...
  for (size_t i = 0; i < count; i++) {
    DS18B20.getAddress(dsDevice[i].addr, i);
    dsDevice[i].strAddr = "";
    char str[5];
    for (size_t j = 0; j < 8; j++) {
      sprintf(str, "%02X", dsDevice[i].addr[j]);
      dsDevice[i].strAddr += str;
      if (j < 7) dsDevice[i].strAddr += "-";
    }
    sensors[dsDevice[i].strAddr] = "";        //add item
  }
